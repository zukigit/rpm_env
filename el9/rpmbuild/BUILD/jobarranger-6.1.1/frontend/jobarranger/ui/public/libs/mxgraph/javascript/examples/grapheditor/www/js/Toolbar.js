	...
	if(this.editorUi.editor.graph.graphScreen != jaConstants.JOBNET_EXEC_GRAPH){
		if (sw >= 400)
		{
				...
				this.edgeStyleMenu = this.addMenuFunction('geSprite-orthogonal', mxResources.get('waypoints'), false, mxUtils.bind(this, function(menu)
			{
				...
				this.editorUi.menus.edgeStyleChange(menu, '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE], ['elbowEdgeStyle', '1', null], 'geIcon geSprite geSprite-curved', null, true).setAttribute('title', mxResources.get('curved')); //
		...
		mxEvent.addListener(elt, 'click', mxUtils.bind(this, function(evt)
		{
			if (show && (elt.enabled == null || elt.enabled))
			{
				...
				menu.hideMenu = mxUtils.bind(this, function()
				{
					...
					menu.destroy();
					let uiTooltip = $(".ui-tooltip");
					if(uiTooltip.is(":visible")){
						uiTooltip.css('visibility', 'hidden');
						uiTooltip.html('');
					}
				});

		 ...
		 if (cand.length > 0)
		 {
			 ...
			 for (var i = 0; i < cand.length; i++)
			 {
				 if (graph.view.getState(cand[i]) != null)
				 {
						if(cand[i].edge){
							let edgeType2 = "";
							if(cand[i].edgeType == 0){
								edgeType2 = "normalFlow";
							}else if(cand[i].edgeType == 1){
								edgeType2 = "trueFlow";
							}else if(cand[i].edgeType == 2){
								edgeType2 = "falseFlow";
							}

							if(cand[i].target != null && cand[i].source != null){
								cand[i].source.outgoingEdge[edgeType2] = cand[i].source.outgoingEdge[edgeType2] + 1;
								cand[i].target.incomingEdge[edgeType2] = cand[i].target.incomingEdge[edgeType2] + 1;
							}else{
								let source = graph.getModel().getCell(cand[i].sourceId);
								let target = graph.getModel().getCell(cand[i].targetId);
							...
							if(cand[i].cellType == jaConstants.ICON_TYPE_START){
								graph.isStartIconExist = true;
							}
							increaseIconId(cand[i].cellType, cand[i].jobId, graph, cand[i].iconSetting);
						}
					 cells.push(cand[i]);
				 }
			 }

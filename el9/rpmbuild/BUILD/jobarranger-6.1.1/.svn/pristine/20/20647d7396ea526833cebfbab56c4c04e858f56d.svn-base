...
import { resetReleaseTooltip } from "../../../../views/jobnetForm/ArrangeIconData";

const ReleaseIconDialog = ({
  id,
  graph,
  visible = true,
  cell,
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      unholdJobId: cell.iconSetting.releaseUnholdJobId || "",
    });
  }, [dispatch]);
  ...
  const onFinish = (values) => {
    ...
    graph.model.setIconSetting(cell, {
      ...
      releaseUnholdJobId: form.getFieldValue("unholdJobId"),
    });
    graph.model.setTooltipLabel(cell, resetReleaseTooltip(cell, t));
    dispatch(removeOpenDialog({ id, graphIndexId }));
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            <Row className="mt-top-2px">
              <Col span={6}>{t("txt-unhold-job-id")}</Col>
              <Col span={18}>
                <Form.Item
                  name="unholdJobId"
                  rules={[
...
export default ReleaseIconDialog;

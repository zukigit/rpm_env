...
Actions.prototype.init = function () {
	...
	pathArray[2] == 'schedule' ? this.addAction('undo', function () { }, null, 'sprite-undo', Editor.ctrlKey + '+Z') : this.addAction('undo', function () { ui.undo(); }, null, 'sprite-undo', Editor.ctrlKey + '+Z');
	...
	this.addAction('redo', function () { ui.redo(); }, null, 'sprite-redo');
	this.addAction('cut', function () { mxClipboard.cut(graph); }, null, 'sprite-cut', Editor.ctrlKey + '+X');
	pathArray[2] == 'schedule' ? this.addAction('copy', function () { }, null, 'sprite-copy', Editor.ctrlKey + '+C') : this.addAction('copy', function () {
		try {
	...
	pathArray[2] == 'schedule' ? this.addAction('paste', function () { }, false, 'sprite-paste', Editor.ctrlKey + '+V') : this.addAction('paste', function () {
	...
	this.addAction('duplicate', function () {
		try {
			//updated by dat at 2022/09/14 for disable duplicate
	...
	this.addAction('lockUnlock', function () {
		//updated by dat at 2022/09/14 for disable lock feature
		// if (!graph.isSelectionEmpty())
		// {
		// 	graph.getModel().beginUpdate();
		// 	try
	...
	this.addAction('setting', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('btn-setting')) != -1) {
				var state = graph.view.getState(cells[0]);
				graph.dblClick(undefined, state);
			}
		}
	}, null, null, 'S');
	this.addAction('jobRun', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('btn-job-run')) != -1) {
				if (cells[0].cellType == jaConstants.ICON_TYPE_JOB) {
					// reinitializeJobId(graph.graphScreen)
					graph.singleJobRun(cells.jobId, cells[0].iconSetting, cells[0].methodFlag);
				}
			}
		}
	}, null, null, 'J');
	this.addAction('methodFlagHold', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('btn-hold')) != -1) {
				cells[0].methodFlag = 1;
				graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ba55d3', [cells[0]])
				graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#ffffff', [cells[0]])
			}
		}
	}, null, null, 'H');
	this.addAction('methodFlagUnhold', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells[0].methodFlag == 1) {
			if (cells.length != 0) {
				if (cells[0].actionEnable.indexOf(t('btn-unhold')) != -1) {
					cells[0].methodFlag = 0;
					graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ffffff', [cells[0]])
					graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#000000', [cells[0]])
				}
			}
		}
	}, null, null, 'U');
	this.addAction('methodFlagSkip', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('sel-skip')) != -1) {
				cells[0].methodFlag = 2;
				graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#808080', [cells[0]])
				graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#ffffff', [cells[0]])
			}
		}
	}, null, null, 'K');
	this.addAction('methodFlagUnskip', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells[0].methodFlag == 2) {
			if (cells.length != 0) {
				if (cells[0].actionEnable.indexOf(t('sel-unskip')) != -1) {
					cells[0].methodFlag = 0;
					graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ffffff', [cells[0]])
					graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#000000', [cells[0]])
				}
	...
	this.addAction('methodFlagTrue', function () {
		var cells = graph.getSelectionCells();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf('True') != -1) {
				cells[0].edgeType = 1;
				cells[0].value = 'True';
				cells[0].target.incomingEdge.trueFlow = 1;
				cells[0].target.incomingEdge.falseFlow = 0;
				if (cells[0].source.outgoingEdge.trueFlow > 0 && cells[0].source.outgoingEdge.falseFlow > 0) {
					for (var i = 0; i < cells[0].parent.children.length; i++) {
						if (cells[0].parent.children[i].dValue == undefined) {
							if (cells[0].parent.children[i].sourceId == cells[0].sourceId && cells[0].parent.children[i].targetId != cells[0].targetId) {
								cells[0].parent.children[i].edgeType = 2;
								cells[0].parent.children[i].value = 'False';
								cells[0].parent.children[i].target.incomingEdge.trueFlow = 0;
								cells[0].parent.children[i].target.incomingEdge.falseFlow = 1;
							}
						}
					}
				} else {
					cells[0].source.outgoingEdge.trueFlow = 1;
					cells[0].source.outgoingEdge.falseFlow = 0;
				}
				graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ffffff', [cells[0]])
				graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#000000', [cells[0]])
				graph.selectAll();
	...
	this.addAction('methodFlagFalse', function () {
		var cells = graph.getSelectionCells();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf('False') != -1) {
				cells[0].edgeType = 2;
				cells[0].value = 'False';
				cells[0].target.incomingEdge.trueFlow = 0;
				cells[0].target.incomingEdge.falseFlow = 1;
				if (cells[0].source.outgoingEdge.trueFlow > 0 && cells[0].source.outgoingEdge.falseFlow > 0) {
					for (var i = 0; i < cells[0].parent.children.length; i++) {
						if (cells[0].parent.children[i].dValue == undefined) {
							if (cells[0].parent.children[i].sourceId == cells[0].sourceId && cells[0].parent.children[i].targetId != cells[0].targetId) {
								cells[0].parent.children[i].edgeType = 1;
								cells[0].parent.children[i].value = 'True';
								cells[0].parent.children[i].target.incomingEdge.trueFlow = 1;
								cells[0].parent.children[i].target.incomingEdge.falseFlow = 0;
							}
						}
					}
				} else {
					cells[0].source.outgoingEdge.trueFlow = 0;
					cells[0].source.outgoingEdge.falseFlow = 1;
				}
				graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ffffff', [cells[0]])
				graph.setCellStyles(mxConstants.STYLE_FONTCOLOR, '#000000', [cells[0]])
				graph.selectAll();
	...
	this.addAction('hold', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('btn-hold')) != -1) {
				graph.setHoldJob(cells[0].innerJobnetId, cells[0].innerJobId);
			}
		}
	}, null, null, 'W');
	this.addAction('unhold', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t('btn-unhold')) != -1) {
				graph.setNormalJob(cells[0].innerJobnetId, cells[0].innerJobId, cells[0].runStatus);
			}
		}
	}, null, null, 'D');
	this.addAction('skip', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t("sel-skip")) != -1) {
				graph.setSkipJob(cells[0].innerJobnetId, cells[0].innerJobId, cells[0].runStatus);
			}
		}
	}, null, null, 'i');
	this.addAction('unskip', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t("sel-unskip")) != -1) {
	...
	this.addAction('forceStop', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells.length != 0) {
				if (cells[0].actionEnable.indexOf(t("btn-force-stop")) != -1) {
					graph.setForceStop(cells[0].innerJobnetId, cells[0].innerJobId);
				}
			}
		}
	}, null, null, 'F');
	this.addAction('rerun', function (evt) {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t("btn-rerun")) != -1) {
				graph.setRerun(cells[0].innerJobnetId, cells[0].innerJobId);
			}
		}
	}, null, null, 'A');
	this.addAction('variableValueChange', function () {
		var cells = graph.getSelectionCells();
		const t = graph.getTranslateObject();
		if (cells.length != 0) {
			if (cells[0].actionEnable.indexOf(t("menu-view-var-value")) != -1) {
				graph.variableValueChange(cells[0].innerJobnetId, cells[0].innerJobId, cells);
			}
		}
	}, null, null, 'C');
	this.addAction('viewVariableValue', function () {
		var cells = graph.getSelectionCells();
		graph.viewVariableValue(cells);
	}, null, null, 'V');
	// Adds action

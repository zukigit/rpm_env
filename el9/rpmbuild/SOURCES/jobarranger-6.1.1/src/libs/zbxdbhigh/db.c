 ...
 *                                                                            *
 ******************************************************************************/
/* issue	: #175 (Reconnect Database Successful)
 *Declare 	: ja_boolean is_reconnect_db and assign JA_FALSE,,
 *Condition : if reconnecting process is success then the INFO log will display "Database started. Reconnect successful."
 *
 * */
int	DBconnect(int flag)
{
	...
	int		err;
	ja_boolean is_reconnect_db = JA_FALSE;
			...
			CONFIG_DBNAME, CONFIG_DBSCHEMA, CONFIG_DBSOCKET, CONFIG_DBPORT)))
	{
		...
		is_reconnect_db= JA_TRUE;

	}

	if( JA_TRUE == is_reconnect_db)
	{
		zabbix_log(LOG_LEVEL_INFORMATION, "Database started. Reconnect successful.");
	}
...
DB_RESULT	__zbx_DBselect(const char *fmt, ...)
{
	...
	while ((DB_RESULT)ZBX_DB_DOWN == rc)
	{
		...
		if ((DB_RESULT)ZBX_DB_DOWN == (rc = zbx_db_vselect(fmt, args)))
		{
			...
			zabbix_log(LOG_LEVEL_INFORMATION, "Database retrying is succeed!");
		}
	}
...
const char	*zbx_user_string(zbx_uint64_t userid)
{
	...
	result = DBselect("select name,surname,username from users where userid=" ZBX_FS_UI64,
			userid);

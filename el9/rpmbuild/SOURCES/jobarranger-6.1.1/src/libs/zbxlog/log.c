...
/* Add message item "[error level]" 2012/10/30 FTF */
			switch(level)
			{
				case LOG_LEVEL_CRIT:
					fprintf(log_file,"[CRIT] ");
					break;
				case LOG_LEVEL_ERR:
...
#if !defined(_WINDOWS)
	else if (LOG_TYPE_CONSOLE == log_type)
	{
		zbx_mutex_lock(&log_file_access);
		gettimeofday(&current_time,NULL);
		tm = localtime(&current_time.tv_sec);
...
// Use this to try system call result to syslog or let use log file bind on daemon.
int ja_system_call(char *cmd){
	if (log_type == LOG_TYPE_SYSLOG){
		// JA_MAX_DATA_LEN = 4096
		char redirect_cmd[(4096 *2)+18];
		 zbx_snprintf(redirect_cmd, sizeof(redirect_cmd),"%s 2>&1 | logger &",cmd);
		return system(redirect_cmd);
	}
	return system(cmd);
}
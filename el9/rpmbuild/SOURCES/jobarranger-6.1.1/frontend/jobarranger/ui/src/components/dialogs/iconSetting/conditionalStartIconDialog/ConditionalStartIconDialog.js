...
import "./ConditionalStartIconDialog.scss";
import {
...
import { resetIfIconTooltip } from "../../../../views/jobnetForm/ArrangeIconData";
const { Option } = Select;

const ConditionalStartIconDialog = ({
  id,
  graph,
  visible = true,
  cell,
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      variable: cell.iconSetting.hasOwnProperty("variable")
        ? cell.iconSetting.variable || ""
        : "",
      processingMethod: cell.iconSetting.hasOwnProperty("handFlag")
        ? cell.iconSetting.handFlag || 0
        : 0,
      comparisonValue: cell.iconSetting.hasOwnProperty("comparisonValue")
        ? cell.iconSetting.comparisonValue || ""
        : "",
    });
  }, [dispatch]);
  ...
  const onFinish = (values) => {
    ...
    graph.model.setIconSetting(cell, {
      ...
      variable: form.getFieldValue("variable"),
      handFlag: form.getFieldValue("processingMethod"),
      comparisonValue: form.getFieldValue("comparisonValue"),
    });
    graph.model.setTooltipLabel(cell, resetIfIconTooltip(cell, t));
    dispatch(removeOpenDialog({ id, graphIndexId }));
  };
                  ...
                  rules={[
                    {
                      required: true,
                      message: t("err-field-required", {
                        field: t("txt-proc-mtd"),
                      }),
                    },
                  ]}
                >
                  <Select>
                    <Option value={0}>{t("txt-num-val")}</Option>
                    <Option value={1}>{t("txt-char-str")}</Option>
                  </Select>
                </Form.Item>
              </Col>
            </Row>
            <Row className="mt-top-2px">
              <Col span={6}>{t("txt-comp-val")}</Col>
              <Col span={18}>
                <Form.Item
                  ...
                  rules={[
                    {
                      ...
                      required: true,
                      message: t("err-field-required", {
                        field: t("txt-comp-val"),
                      }),
                    },
                    ({ getFieldValue }) => ({
                      validator(_, value) {
                        if (getFieldValue("processingMethod") === 0) {
                          var regexComma = new RegExp(
                            REGEX_PATTERM.MATCH_HANKAKU_COMMA_HYPHEN
                          );
                          if (!regexComma.test(value)) {
                            return Promise.reject(
                              new Error(
...
export default ConditionalStartIconDialog;

...
//		var tp = mxUtils.getValue(this.style, 'tabPosition', this.tabPosition);
	...
	TrapezoidShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		...
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		// console.log("HHHHHHH h:" + h + " w:" + w + " dx:" + dx);
		this.addPoints(c, [new mxPoint(w - dx, h), new mxPoint(w, 0), new mxPoint(0, 0), new mxPoint(dx, h)],
				this.isRounded, arcSize, true);
	};
	...
	HexagonShape.prototype.size = 0.15;
	...
	//MTK Jobnet Icon
	function HexagonShape1()
	{
		mxActor.call(this);
	};
	mxUtils.extend(HexagonShape1, mxHexagon);
	HexagonShape1.prototype.size = 0.15;
	HexagonShape1.prototype.fixedSize = 20;
	HexagonShape1.prototype.isRoundable = function()
	{
		return true;
	};
	HexagonShape1.prototype.redrawPath = function(c, x, y, w, h)
	{
		var fixed = mxUtils.getValue(this.style, 'fixedSize', '0') != '0';
		var s = (fixed) ? Math.max(0, Math.min(w * 0.5, parseFloat(mxUtils.getValue(this.style, 'size', this.fixedSize)))) :
			w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(s, 0), new mxPoint(w - s, 0), new mxPoint(w, 0.35 * h),new mxPoint(w, 0.65 * h),
		 new mxPoint(w - s, h), new mxPoint(s, h),new mxPoint(0, 0.65 * h),new mxPoint(0, 0.35 * h)], this.isRounded, arcSize, true);
	};

	mxCellRenderer.registerShape('jobnet', HexagonShape1);

	//MTK release icon
	function HexagonShape2()
	{
		mxActor.call(this);
	};
	mxUtils.extend(HexagonShape2, mxHexagon);
	HexagonShape2.prototype.size = 0.15;
	HexagonShape2.prototype.fixedSize = 20;
	HexagonShape2.prototype.isRoundable = function()
	{
		return true;
	};
	HexagonShape2.prototype.redrawPath = function(c, x, y, w, h)
	{
		var fixed = mxUtils.getValue(this.style, 'fixedSize', '0') != '0';
		var s = (fixed) ? Math.max(0, Math.min(w * 0.5, parseFloat(mxUtils.getValue(this.style, 'size', this.fixedSize)))) :
			w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, 0), new mxPoint(w-s, 0), new mxPoint(w, 0.2*h),new mxPoint(w, h),
			new mxPoint(s, h),new mxPoint(0, 0.8*h)], this.isRounded, arcSize, true);
	};

	mxCellRenderer.registerShape('release', HexagonShape2);
	...
	//job icon and other rectangle shapes
	...
	//Delete nodes for internal shapes
	...
	//IF icon, End if icon
	mxRhombus.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0, 0.5), true),
	                                   new mxConnectionConstraint(new mxPoint(0.5, 0), true),
	                                   new mxConnectionConstraint(new mxPoint(0.5, 1), true),
	                                   new mxConnectionConstraint(new mxPoint(1, 0.5), true)];
	...
	//Task icon and Jobnet icon
	mxHexagon.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0.5, 0), false),
										new mxConnectionConstraint(new mxPoint(1, 0.5), false),
										new mxConnectionConstraint(new mxPoint(0.5, 1), false),
										new mxConnectionConstraint(new mxPoint(0, 0.5), false)];
	...
	//Variable icon
	ParallelogramShape.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0, 0.5), true),
	                                   			new mxConnectionConstraint(new mxPoint(0.5, 0), true),
	                                   			new mxConnectionConstraint(new mxPoint(0.5, 1), true),
	                                   			new mxConnectionConstraint(new mxPoint(1, 0.5), true)];
	//Calculation icon
	TrapezoidShape.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0.5, 0), true),
	        	              		 		new mxConnectionConstraint(new mxPoint(0, 0.5), true),
	        	            		 		new mxConnectionConstraint(new mxPoint(1, 0.5), true),
	        	            				new mxConnectionConstraint(new mxPoint(0.5, 1), true)];
	//File transfer, file wait

...
class SetupRequire extends Controller
{
	...
	public function checkPhpDatabases()
	{
		...
		return [
			...
			'current' => empty($current) ? _('off') : $current,
			'required' => null,
	...
	// 	$databases = self::getSupportedDatabases();
	// 	foreach ($databases as $name) {
	// 		$current .='<option value="'.strtolower($name).'">'.$name.'</option>';
	// 	}
	// 	return $current;
	// }
	...
	public static function getSupportedDatabases()
	{
		...
		if (zbx_is_callable([
			'mysqli_close', 'mysqli_connect', 'mysqli_connect_error', 'mysqli_error',
		...
		if (zbx_is_callable([
			'pg_close', 'pg_connect', 'pg_escape_bytea', 'pg_escape_string', 'pg_fetch_assoc',
		...
		// if (zbx_is_callable([
		// 	'oci_bind_by_name', 'oci_close', 'oci_commit', 'oci_connect', 'oci_error', 'oci_execute',

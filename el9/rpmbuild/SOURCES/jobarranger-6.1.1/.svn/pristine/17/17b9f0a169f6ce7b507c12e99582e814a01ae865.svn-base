	...
	if (this.factoryMethod == null) {
		...
		if (source != null && target != null) {
			if (source.cellType == jaConstants.ICON_TYPE_CONDITIONAL_START) {
				let btnTrue = true;
				let btnFalse = true;
				if (source2.outgoingEdge != null) {
					if (source2.outgoingEdge.hasOwnProperty("trueFlow")) {
						if (source2.outgoingEdge.trueFlow > 0) {
							btnTrue = false;
						}
					}
					if (source2.outgoingEdge.hasOwnProperty("falseFlow")) {
						if (source2.outgoingEdge.falseFlow > 0) {
							btnFalse = false;
						}
					}
				}
				if (!(btnTrue && btnFalse)) {
					btnFalse ? graph2.insertEdge(parent2, id2, "False", source2, target2, style2, 2) : graph2.insertEdge(parent2, id2, "True", source2, target2, style2, 1)
				} else {
					const t = graph2.getTranslateObject();
					$.confirm({
						columnClasses: 'small',
						title: t("flow-type"),
						content: (btnTrue || btnFalse) ? t('cho-flow-type') : t('Sorry, true flow and false flow are already drawn'),
						type: 'blue',
						closeIcon: true,
						buttons: {
							True: {
								btnClass: btnTrue ? '' : 'disabled',
								action: function () {
									graph2.insertEdge(parent2, id2, "True", source2, target2, style2, 1);
								}
							},
							False: {
								btnClass: btnFalse ? '' : 'disabled',
								action: function () {
									graph2.insertEdge(parent2, id2, "False", source2, target2, style2, 2);
								}
							},
						}
					});

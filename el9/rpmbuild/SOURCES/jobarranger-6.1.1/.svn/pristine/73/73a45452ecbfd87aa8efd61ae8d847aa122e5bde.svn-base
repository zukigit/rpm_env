...
mxGraphView.prototype.updateCellState = function(state)
{
	...
	// if(state.cell.cellType == jaConstants.ICON_TYPE_CALCULATION){
	// 	state.style.rotation = 360;
	// }
	...
	if (state.cell != this.currentRoot)
	{
		...
		if (geo != null)
		{
			...
			if(this.translate.x == 29 || this.translate.x == 39){
				state.x = this.scale * (0 + state.origin.x);
			}else{
...
mxGraphView.prototype.updatePoints = function(edge, points, source, target)
{
	if (edge != null)
	{
		...
		if (edgeStyle != null)
		{
			...
			// curve flow point calculation
			if(edge.cell.style.includes("curved=1") && edge.cell.style.includes("db=1")){

				if(edge.cell.source != null && edge.cell.target != null){
					
					var midPoint = new mxPoint((edge.absolutePoints[0].x + edge.absolutePoints[1].x)/2, (edge.absolutePoints[0].y + edge.absolutePoints[1].y)/2)

					const angleFunc = (anchor, point) => Math.atan2(anchor.y - point.y, anchor.x - point.x) * 180 / Math.PI + 180;
					let angleDegree = angleFunc(edge.absolutePoints[1], edge.absolutePoints[0]); 
					let slopeAngle = false;
					if((angleDegree > 20 && angleDegree < 70) || (angleDegree > 110 && angleDegree < 160) || (angleDegree > 200 && angleDegree < 250) || (angleDegree > 290 && angleDegree < 340)){
						slopeAngle = true;
					}

					var angle = (angleDegree - 270) / 180 * Math.PI;  // compensate angle -90°, conv. to rad
					if(slopeAngle){
						midPoint.x += 150 * Math.cos(angle);          // move ship
						midPoint.y += 120 * Math.sin(angle);
					}else{
						midPoint.x += 45 * Math.cos(angle);          // move ship
						midPoint.y += 45 * Math.sin(angle);
					}

					if(points == null){
						points = new Array(midPoint)
						edge.cell.geometry.points = new Array(midPoint)
					}
				}
				edge.cell.style = edge.cell.style.replace("db=1;", "");
			}

...
mxEdgeHandler.prototype.connect = function(edge, terminal, isSource, isClone, me)
{
	...
	let moveEdge = terminal;
	let cells = [edge];
	let i = 0;
	
	if(isSource){
		if(moveEdge.id == source.id){
			result = true;
		}else if(moveEdge.id == target.id || moveEdge.dValue == jaConstants.CHILD_ICON){
			result = false;
		}else{
	
		//check source is valid
		if (moveEdge != null && target != null) {
			if (
				moveEdge.cellType == jaConstants.ICON_TYPE_JOB ||
				moveEdge.cellType == jaConstants.ICON_TYPE_JOBNET ||
				moveEdge.cellType == jaConstants.ICON_TYPE_JOB_CONTROL_VARIABLE ||
				moveEdge.cellType == jaConstants.ICON_TYPE_EXTENDED_JOB ||
				moveEdge.cellType == jaConstants.ICON_TYPE_START ||
				moveEdge.cellType == jaConstants.ICON_TYPE_CALCULATION ||
				moveEdge.cellType == jaConstants.ICON_TYPE_LOOP ||
				moveEdge.cellType == jaConstants.ICON_TYPE_TASK ||
				moveEdge.cellType == jaConstants.ICON_TYPE_INFO ||
				moveEdge.cellType == jaConstants.ICON_TYPE_PARALLEL_END ||
				moveEdge.cellType == jaConstants.ICON_TYPE_CONDITIONAL_END ||
				moveEdge.cellType == jaConstants.ICON_TYPE_FILE_COPY ||
				moveEdge.cellType == jaConstants.ICON_TYPE_FILE_WAIT ||
				moveEdge.cellType == jaConstants.ICON_TYPE_REBOOT ||
				moveEdge.cellType == jaConstants.ICON_TYPE_ZABBIX ||
				moveEdge.cellType == jaConstants.ICON_TYPE_AGENT_LESS ||
				moveEdge.cellType == jaConstants.ICON_TYPE_RELEASE
				) {
					// check flow is normal flow
					if (cells[i].edgeType == 0) {
					if (moveEdge.outgoingEdge.normalFlow == 0) {
				...
				if (moveEdge.cellType == jaConstants.ICON_TYPE_PARALLEL_START) {
					// check flow is normal flow
					if (cells[i].edgeType == 0) {
					outgoingEdgeCount = outgoingEdgeCount + 1;
					sourceValid = true;
				...
				if (moveEdge.cellType == jaConstants.ICON_TYPE_CONDITIONAL_START) {
				// check flow is true flow or false flow
					if (cells[i].edgeType == 1 || cells[i].edgeType == 2) {
						if (cells[i].edgeType == 1) {
							if (moveEdge.outgoingEdge.trueFlow == 0) {
								outgoingEdgeCount = outgoingEdgeCount + 1;
								sourceValid = true;
							}
						} else if (cells[i].edgeType == 2) {
							if (moveEdge.outgoingEdge.falseFlow == 0) {
								outgoingEdgeCount = outgoingEdgeCount + 1;
								sourceValid = true;
							}
						}
			...
			if (outgoingEdgeCount != null) {
				if(cells[i].edgeType == 0){
					moveEdge.outgoingEdge.normalFlow = outgoingEdgeCount;
				}else if(cells[i].edgeType == 1){
					moveEdge.outgoingEdge.trueFlow = outgoingEdgeCount;
				}else if(cells[i].edgeType == 2){
					moveEdge.outgoingEdge.falseFlow = outgoingEdgeCount;
				}
			}
			...
			if(source.outgoingEdge[edgeType] != 0){
				source.outgoingEdge[edgeType] = source.outgoingEdge[edgeType] - 1;
			}
		...
		if(moveEdge.id == target.id){
			result = true;
		}else if(moveEdge.id == source.id || moveEdge.dValue == jaConstants.CHILD_ICON){
			result = false;
		}else{
	
			//check target is valid
		if (source != null && moveEdge != null) {
			if (
				moveEdge.cellType == jaConstants.ICON_TYPE_JOB ||
				moveEdge.cellType == jaConstants.ICON_TYPE_JOBNET ||
				moveEdge.cellType == jaConstants.ICON_TYPE_END ||
				moveEdge.cellType == jaConstants.ICON_TYPE_JOB_CONTROL_VARIABLE ||
				moveEdge.cellType == jaConstants.ICON_TYPE_EXTENDED_JOB ||
				moveEdge.cellType == jaConstants.ICON_TYPE_CALCULATION ||
				moveEdge.cellType == jaConstants.ICON_TYPE_LOOP ||
				...
				moveEdge.cellType == jaConstants.ICON_TYPE_REBOOT ||
				moveEdge.cellType == jaConstants.ICON_TYPE_RELEASE ||
				moveEdge.cellType == jaConstants.ICON_TYPE_PARALLEL_START ||
				moveEdge.cellType == jaConstants.ICON_TYPE_CONDITIONAL_START ||
				moveEdge.cellType == jaConstants.ICON_TYPE_ZABBIX ||
				moveEdge.cellType == jaConstants.ICON_TYPE_AGENT_LESS
			) {
				// if (cells[i].edgeType == 0) {
					let totalCount = moveEdge.incomingEdge.normalFlow + moveEdge.incomingEdge.trueFlow + moveEdge.incomingEdge.falseFlow;
				if (moveEdge.cellType == jaConstants.ICON_TYPE_LOOP) {
					if (totalCount <= 1) {
			...
			if (incomingEdgeCount != null) {
				if(cells[i].edgeType == 0){
					moveEdge.incomingEdge.normalFlow = incomingEdgeCount;
				}else if(cells[i].edgeType == 1){
					moveEdge.incomingEdge.trueFlow = incomingEdgeCount;
				}else if(cells[i].edgeType == 2){
					moveEdge.incomingEdge.falseFlow = incomingEdgeCount;
				}
			}
			...
			if(target.incomingEdge[edgeType] != 0){
				target.incomingEdge[edgeType] = target.incomingEdge[edgeType] - 1;
			}
	...
	if(result){
		...
		try
		{
			...
			if(!this.graph.isGraphChange){
				this.graph.isGraphChange = true;
				this.graph.detectGraphChange();
			}
			
			//connection constraint East/West/North/South
			// var constraint = [new mxConnectionConstraint(new mxPoint(0.5, 0), true), new mxConnectionConstraint(new mxPoint(0, 0.5), true), new mxConnectionConstraint(new mxPoint(1, 0.5), true), new mxConnectionConstraint(new mxPoint(0.5, 1), true),];
...
{
	if(points.length > 9){
		this.graph.alertError("Sorry, maximum corner of flow must be 9.");
		return;
	 }
	var model = this.graph.getModel();
	model.beginUpdate();

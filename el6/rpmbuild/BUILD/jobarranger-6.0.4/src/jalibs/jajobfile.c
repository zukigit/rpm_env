...
char JA_IPS_FOLDER[JA_FILE_PATH_LEN];
 ...
 * Function:                                                                  *
 *                                                                            *
 * Purpose: to remove aborting job flag file                                  *
 *                                                                            *
 * Parameters:                                                                *
 *                                                                            *
 * Return value:                                                              *
...
void ja_remove_abort_job_file(zbx_uint64_t jobid){
    ...
    char filename[JA_MAX_STRING_LEN];
    const char *__function_name = "ja_remove_abort_job_file";
    ...
    while(result != SUCCEED){
        ...
        if(result == SUCCEED){
            zabbix_log(LOG_LEVEL_INFORMATION, "In %s(), abort_job_flag file:%s have been removed." ,
                        __function_name,filename);
        }else {
            if(fopen(filename, "r") == NULL){
                zabbix_log(LOG_LEVEL_ERR, "In %s(), abort_job_flag file:"ZBX_FS_UI64" does not exist in [%s]. (%s).",
                            __function_name, jobid,folderpath , strerror(errno));
            }else{
                zabbix_log(LOG_LEVEL_ERR, "In %s(), abort_job_flag file:%s cannot be deleted. (%s).",
                            __function_name,folderpath , strerror(errno));
            }
        }                
        if (cnt > 3) {
            zabbix_log(LOG_LEVEL_INFORMATION, "In %s(), Deleting the abort_job_flag file:%s is skipped...... (%s).",
                        __function_name,folderpath , strerror(errno));
            result = SUCCEED;
        }
    }
...
int ja_jobfile_load(const char *filepath, ja_job_object * job)
{
    ...
    // } else {
    //     if (ja_file_load(filename, sizeof(res_ret), &res_ret) == FAIL) {
    //         zabbix_log(LOG_LEVEL_INFORMATION,"Debug: In %s(), ja_file_load has failed!!");
    //         //if job type is reboot process, set exit code as zero.
    //         if (strcmp(job->type, JA_PROTO_VALUE_REBOOT) == 0) {
    //             res_ret = 0;
    //         }else{
...
#endif
int ja_jobfile_check_processexist(char *filename,char *full_filename,char* cur_dir) 
{
    DIR* exec_dir;
    struct dirent* entry;
    ...
    char* substr;
    int proc_run_flag = 0,jobid_len = 0;
    ...
    int ii = 0;
    jobid_len = 0;
    while(jobid_datetime[ii] != '-' && ii<= strlen(full_filename)){
        jobid[ii] = jobid_datetime[ii];
        ii++;
        jobid_len++;
    }
        
    char* chk_folder[] = { JA_TEMP_FOLDER, JA_BEGIN_FOLDER, JA_EXEC_FOLDER, JA_END_FOLDER,NULL };
    int i = 0;
    while(chk_folder[i] != NULL && i<5){
        if(cur_dir != NULL && strcmp(chk_folder[i],cur_dir) ==0){
            i++;
            continue;
        }
        exec_dir = opendir(chk_folder[i]);
        i++;
        ...
        while (NULL != (entry = readdir(exec_dir)))
        {
            ...
            if (strncmp( entry->d_name,jobid, jobid_len) == 0) {
                zbx_snprintf(filename, strlen(entry->d_name)+1, "%s", entry->d_name);
                proc_run_flag = 1;
                break;
            }
        }
        closedir(exec_dir);
        if(proc_run_flag == 1){
            zabbix_log(LOG_LEVEL_ERR, "In %s(), executing process found. dir :%s, filename:%s", __function_name,chk_folder[i-1],filename);
            break;
        }
    }
contd:
...
void ja_jobfile_fullpath(char* full_filename, int folder_type, char* result_filepath) {
    ...
    case JA_TYPE_STATUS_JOBS:
        ...
        break;
    case JA_TYPE_STATUS_IPS:
        zbx_snprintf(result_filepath, JA_FILE_PATH_LEN, "%s%c%s", JA_IPS_FOLDER, JA_DLM, full_filename);
        break;
    default:

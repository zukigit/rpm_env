...
static void jobarg_load_config(int requirement)
{
    ...
    zbx_snprintf(JA_IPS_FOLDER, sizeof(JA_IPS_FOLDER), "%s%c%s", CONFIG_TMPDIR, JA_DLM, "serverIPs");
    zbx_snprintf(JA_JOBS_FOLDER, sizeof(JA_JOBS_FOLDER), "%s%c%s", CONFIG_TMPDIR, JA_DLM, "abort");
}
...
DWORD WINAPI tmpFolderPrepare(LPVOID lpParam) {
    ...
    int tmp_cnt = 0,iia = 0;
    char* tmp_folder[] = {"temp","begin","close","data","end","error","exec","jobs","serverIPs","abort", NULL};
    ...
    while (tmp_folder[tmp_cnt] != NULL) {
        ...
        if(tmp_cnt != JA_TYPE_STATUS_IPS) {
            ...
            zbx_snprintf(tmp_file, sizeof(tmp_file), "%s%cserverIPs.json", folderpath, JA_DLM);
...
int MAIN_ZABBIX_ENTRY()
{
    ...
    while (tmp_folder[tmp_cnt] != NULL) {
        ...
        if(tmp_cnt != JA_TYPE_STATUS_IPS) {
            ...
            zbx_snprintf(tmp_file, sizeof(tmp_file), "%s%cserverIPs.json", folderpath, JA_DLM); //file name for serverIPs folder
    ...
    res =
        WaitForMultipleObjectsEx(threads_num, threads, FALSE, INFINITE,
                                 FALSE);
    zabbix_log(LOG_LEVEL_INFORMATION,"One thread has terminated unexpectedly (code:%lu). Exiting ...",res);
    if (ZBX_IS_RUNNING()) {
        zabbix_log(LOG_LEVEL_INFORMATION,

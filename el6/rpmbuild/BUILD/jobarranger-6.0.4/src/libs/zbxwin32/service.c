...
static VOID WINAPI ServiceCtrlHandler(DWORD ctrlCode)
{
	...
	switch(ctrlCode)
	{
		...
		case SERVICE_CONTROL_SHUTDOWN:
			zabbix_log(LOG_LEVEL_INFORMATION, "Job Arranger Agent shutdown requested");
...
void	service_start()
{
	...
	if (0 == ret)
	{
		...
		{
			zbx_error("\n\n\t!!!ATTENTION!!! Job Arranger Agent started as a console application. !!!ATTENTION!!!\n");
			MAIN_ZABBIX_ENTRY();
		}
		else
...
#define MAX_CMD_LEN	MAX_PATH * 2
	...
	else
	{
		...
		{
			sd.lpDescription = TEXT("Provides job scheduling");

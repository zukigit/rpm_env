...
int jarun_status_check(const char *host_name, int *joblist, int count_job,int* reboot_job_list)
{
    ...
    if (hostid == 0) {
        zabbix_log(LOG_LEVEL_WARNING, "In %s(), Can not get host id for %s", __function_name, host_name);
        return FAIL;
    }

    port = ja_host_getport(hostid, 0);
    if(port ==0) {
        zabbix_log(LOG_LEVEL_WARNING, "In %s(), Can not get port for %s", __function_name, host_name);
        return FAIL;
    }

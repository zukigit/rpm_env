...
int jarun_icon_job(zbx_uint64_t inner_job_id, int flag)
{
    ...
    if ( flag == JA_AGENT_METHOD_ABORT || flag == JA_AGENT_METHOD_KILL ) {
        ...
        row = DBfetch(result);
        if( atoi(row[0]) ==  JA_JOB_STATUS_RUN ){
            // zabbix_log(LOG_LEVEL_DEBUG,"In %s(), inner_job_id:"ZBX_FS_UI64" status is changed to JA_JOB_STATUS_ABORT", __function_name, inner_job_id);
            if (ja_set_status_job(inner_job_id, JA_JOB_STATUS_ABORT, -1, -1) == FAIL){
                DBfree_result(result);
                return FAIL;
            }
        }
  ...
  error:
   if(con_err == SUCCEED){
    ...
    if (ret == SUCCEED) {
        if(flag == JA_AGENT_METHOD_KILL || flag == JA_AGENT_METHOD_ABORT)
            zabbix_log(LOG_LEVEL_INFORMATION,"In %s(), Send kill request to agent for inner job id :" ZBX_FS_UI64, __function_name, inner_job_id);
        exit(0);
    }
    else
    {

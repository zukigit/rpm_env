...
int jatrap_jobresult(ja_telegram_object *obj)
{
    ...
    else
    {
        ...
        {
            zabbix_log(LOG_LEVEL_DEBUG, "Duplicate Data %s", previous);
    ...
    else if (status == JA_JOB_STATUS_ABORT && job.signal == 1)
    {
        msg = zbx_dsprintf(NULL, "Aborted the job");
        icon_status = JA_JOB_STATUS_ABORT;
    }
    ...
    if (run_err == 0)
    {
        res1 = ja_set_value_after(job.jobid, inner_jobnet_id, "ICON_STATUS", "0");
        res2 = ja_flow(job.jobid, JA_FLOW_TYPE_NORMAL, 1);
        
		if (res1 == FAIL || res2 == FAIL) {
            zabbix_log(LOG_LEVEL_ERR, "In %s(),Error occured while changing \"after\" status.jobid : "ZBX_FS_UI64,__function_name,job.jobid);
            err = zbx_dsprintf(NULL, "AFTER_STATUS ERROR");
            goto error;          
        }
    }
    else

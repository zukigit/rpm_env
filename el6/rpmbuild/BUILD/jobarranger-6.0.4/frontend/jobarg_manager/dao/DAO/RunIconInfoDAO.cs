...
{
    /// <summary>
    /// 情報取得アイコン設定テーブルのDAOクラス
    /// </summary>
    public class RunIconInfoDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_info_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_info_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_info_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_info_table " +
        ...
        public RunIconInfoDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

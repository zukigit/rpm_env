...
{
    /// <summary>
    /// 実行リブートアイコンテーブルのDAOクラス
    /// </summary>
    public class RunIconRebootDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_reboot_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_reboot_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_reboot_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_reboot_table " +
        ...
        public RunIconRebootDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

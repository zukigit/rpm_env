...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// 計算アイコン設定テーブルのDAOクラス
    /// </summary>
    public class RunIconCalcDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_calc_table";

        private string[] _primaryKey = { "inner_job_id"};

        private string _selectSql = "select * from ja_run_icon_calc_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_calc_table " +
                                        "where inner_job_id = ? ";
        private string _selectSqlByJobnet = "select * from ja_run_icon_calc_table " +
        ...
        public RunIconCalcDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

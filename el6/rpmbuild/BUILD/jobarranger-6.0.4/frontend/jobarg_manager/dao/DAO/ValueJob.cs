...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// ジョブ変数設定テーブルのDAOクラス
    /// </summary>
    public class ValueJobDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_value_job_table";

        private string[] _primaryKey = { "jobnet_id", "job_id", "update_date", "value_name" };

        private string _selectSql = "select * from ja_value_job_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_value_job_table " +
                                        "where jobnet_id = ? " +
                                        "and job_id= ? " +
                                        "and update_date = ? " +
                                        "and value_name = ?";

        private string _selectSqlByJobnet = "select * from ja_value_job_table " +
                        "where jobnet_id = ? " +
                        "and update_date = ?";
        ...
        public ValueJobDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

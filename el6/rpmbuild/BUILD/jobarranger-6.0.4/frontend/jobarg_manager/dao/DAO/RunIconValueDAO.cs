...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// </summary>
    public class RunIconValueDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_value_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_value_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_value_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_value_table " +
        ...
        public RunIconValueDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

...
{
    /// <summary>
    /// Zabbix連携アイコン実行テーブルのDAOクラス
    /// </summary>
    public class RunIconCooperationDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_zabbix_link_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_zabbix_link_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_zabbix_link_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_zabbix_link_table " +
        ...
        public RunIconCooperationDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

...
# install - install a Job Arranger Agent for HP-UX  - 2014/12/12 -
#
# Copyright (C) 2013 Daiwa Institute of Research Business Innovation Ltd. All Rights Reserved.
#
# This installation shell does the standard installation of Job Arranger
# Agent for HP-UX. Advanced options I can be found in the "install.sh -h".
#
...
#  ./sbin/init.d/jobarg-agentd          --> /sbin/init.d/.
#
# Create the following symbolic files.
#
#  /sbin/init.d/jobarg-agentd --> /sbin/rc3.d/S999jobarg-agentd
#  /sbin/init.d/jobarg-agentd --> /sbin/rc0.d/K000jobarg-agentd
...
/usr/bin/cp ./sbin/init.d/jobarg-agentd ${INIT_DIR}/.
if [ $? -ne 0 ] 
then
    echo "failed to copy the ./sbin/init.d/jobarg-agentd file"
    exit -1
fi

ln -f -s ${INIT_DIR}/jobarg-agentd ${RC_3_DIR}/S999jobarg-agentd
if [ $? -ne 0 ] 
then
    echo "failed to create a symbolic link of ${RC_3_DIR}/S999jobarg-agentd file"
    exit -1
fi

ln -f -s ${INIT_DIR}/jobarg-agentd ${RC_0_DIR}/K000jobarg-agentd
if [ $? -ne 0 ] 
then
    echo "failed to create a symbolic link of ${RC_0_DIR}/K000jobarg-agentd file"
    exit -1
fi

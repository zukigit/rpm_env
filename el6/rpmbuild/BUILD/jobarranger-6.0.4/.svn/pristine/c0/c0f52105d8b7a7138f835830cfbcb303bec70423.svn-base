                       ...
                       const zbx_uint64_t end_inner_job_id)
{
    ...
    start_job_result =
        DBselect
        ("select status from ja_run_job_table where inner_job_id = " ZBX_FS_UI64,
            start_inner_job_id);
    if(NULL != (row = DBfetch(start_job_result))){
        if(atoi(row[0]) == JA_JOB_STATUS_ABORT){
            DBfree_result(start_job_result);
            return SUCCEED;
        }
    }
    DBfree_result(start_job_result);

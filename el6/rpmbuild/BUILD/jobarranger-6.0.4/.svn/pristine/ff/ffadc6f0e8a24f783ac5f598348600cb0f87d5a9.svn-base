...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// </summary>
    public class JobCommandDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_job_command_table";

        private string[] _primaryKey = { "jobnet_id", "job_id", "update_date", "command_cls" };

        private string _selectSql = "select * from ja_job_command_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_job_command_table " +
                                        "where jobnet_id = ? " +
                                        "and job_id= ? " +
                                        "and update_date = ? " +
                                        "and command_cls = ?";

        private string _selectSqlByJobnet = "select * from ja_job_command_table " +
                        "where jobnet_id = ? " +
                        "and update_date = ?";
        ...
        public JobCommandDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion
        ...
        /// <return>検索結果</return>
        //************************************************************************
        public DataTable GetEntityByPk(object jobnet_id, object job_id, object update_date, object command_cls)
        {
            _db.CreateSqlConnect();

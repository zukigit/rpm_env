...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// </summary>
    public class RunValueJobConDAO : BaseDAO
    {
        #region フィールド



        private string _tableName = "ja_run_value_jobcon_table";

        private string[] _primaryKey = { "inner_job_id", "value_name" };

        private string _selectSql = "select * from ja_run_value_jobcon_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_value_jobcon_table " +
                                        "where inner_job_id = ? " +
                                        "and value_name = ?";

        private string _selectSqlByJobnet = "select * from ja_run_value_jobcon_table " +
        ...
        public RunValueJobConDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

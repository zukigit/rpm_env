...
{
    /// <summary>
    /// ジョブネットアイコン設定テーブルのDAOクラス
    /// </summary>
    public class IconJobnetDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_icon_jobnet_table";

        private string[] _primaryKey = { "jobnet_id", "job_id", "update_date" };

        private string _selectSql = "select * from ja_icon_jobnet_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_icon_jobnet_table " +
                                        "where jobnet_id = ? " +
                                        "and job_id= ? " +
                                        "and update_date = ?";

        private string _selectSqlByJobnet = "select * from ja_icon_jobnet_table " +
                        "where jobnet_id = ? " +
                        "and update_date = ?";

        private string _selectCounForJobInfo = "select count(1) as count from ja_icon_jobnet_table " +
        ...
        public IconJobnetDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion
        ...
        /// <param name="jobnet_id">ジョブネットID</param>
        /// <param name="update_date">更新日</param>
        /// <param name="link_jobnet_id">リンク先ジョブネットID</param>
        /// <return>検索結果</return>
        //************************************************************************
        public string GetCountForJobInfo(object jobnet_id, object update_date, object link_jobnet_id)
        {
            string count = "";

            List<ComSqlParam> sqlParams = new List<ComSqlParam>();
            sqlParams.Add(new ComSqlParam(DbType.String, "jobnet_id", jobnet_id));
            sqlParams.Add(new ComSqlParam(DbType.String, "update_date", update_date));
            sqlParams.Add(new ComSqlParam(DbType.String, "link_jobnet_id", link_jobnet_id));
            DataTable dt = _db.ExecuteQuery(_selectCounForJobInfo, sqlParams, TableName);

...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// ジョブ管理テーブルのDAO
    /// </summary>
    public class RunJobDAO : BaseDAO
    {
        #region フィールド



        private string _tableName = "ja_run_job_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_job_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_job_table " +
            "where inner_job_id = ?";

        private string _selectSqlByPkForUpdate = "select * from ja_run_job_table " +
            "where inner_job_id = ? for update";

        private string _selectSqlByJobnet = "select * from ja_run_job_table " +
        ...
        /// <return>検索結果</return>
        //************************************************************************
        public DataTable GetEntityByPkForUpdate(object inner_job_id)
        {
            _db.CreateSqlConnect();
            ...
            DataTable dt = _db.ExecuteQuery(this._selectSqlByPkForUpdate, sqlParams, TableName);

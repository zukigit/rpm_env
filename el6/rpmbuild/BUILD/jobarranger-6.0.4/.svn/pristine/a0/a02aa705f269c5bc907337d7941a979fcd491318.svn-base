...
{
    /// <summary>
    /// 実行ファイル転送設定テーブルのDAOクラス
    /// </summary>
    public class RunIconFcopyDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_fcopy_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_fcopy_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_fcopy_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_fcopy_table " +
        ...
        public RunIconFcopyDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

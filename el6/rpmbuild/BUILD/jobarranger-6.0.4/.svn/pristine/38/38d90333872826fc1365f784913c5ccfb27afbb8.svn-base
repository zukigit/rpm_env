...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// フロー管理テーブルのDAOクラス
    /// </summary>
    public class RunFlowDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_flow_table";

        private string[] _primaryKey = { "inner_flow_id" };

        private string _selectSql = "select * from ja_run_flow_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_flow_table " +
                                        "where inner_flow_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_flow_table " +
        ...
        public RunFlowDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion
        ...
        /// <param name="inner_flow_id">実行用フロー内部管理ID</param>
        /// <return>検索結果</return>
        //************************************************************************
        public DataTable GetEntityByPk(object inner_flow_id)
        {
            _db.CreateSqlConnect();

            List<ComSqlParam> sqlParams = new List<ComSqlParam>();
            sqlParams.Add(new ComSqlParam(DbType.String, "@inner_flow_id", inner_flow_id));
        ...
        /// データの取得.
        /// </summary>
        /// <param name="inner_jobnet_id">実行用フロー内部管理ID</param>
        /// <return>検索結果</return>
        //************************************************************************

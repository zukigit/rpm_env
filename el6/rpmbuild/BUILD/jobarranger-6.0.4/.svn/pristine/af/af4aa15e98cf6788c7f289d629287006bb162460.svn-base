...
{
    /// <summary>
    /// 実行エージェントレスアイコン設定テーブルのDAOクラス
    /// </summary>
    public class RunIconAgentlessDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_agentless_table";

        private string[] _primaryKey = { "inner_job_id" };

        private string _selectSql = "select * from ja_run_icon_agentless_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_agentless_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_agentless_table " +
        ...
        public RunIconAgentlessDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    /// 終了アイコン設定テーブルのDAOクラス
    /// </summary>
    public class IconEndDAO : BaseDAO
    {
        #region フィールド



        private string _tableName = "ja_icon_end_table";

        private string[] _primaryKey = { "jobnet_id", "job_id", "update_date" };

        private string _selectSql = "select * from ja_icon_end_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_icon_end_table " +
                                        "where jobnet_id = ? " +
                                        "and job_id= ? " +
                                        "and update_date = ?";

        private string _selectSqlByJobnet = "select * from ja_icon_end_table " +
                        "where jobnet_id = ? " +
                        "and update_date = ?";
        ...
        public IconEndDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

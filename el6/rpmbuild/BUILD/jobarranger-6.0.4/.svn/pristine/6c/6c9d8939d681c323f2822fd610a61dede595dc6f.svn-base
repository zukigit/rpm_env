	...
	else
	{
		if (0 != mysql_query(conn, sql))
		{
			...
			switch (mysql_errno(conn))
			{
				...
				case ER_TABLEACCESS_DENIED_ERROR:/* user without some privilege */
				case ER_UNKNOWN_ERROR:
				case ER_LOCK_DEADLOCK:/*failed to lock*/
			...
			if(mysql_errno(conn) == ER_LOCK_WAIT_TIMEOUT) {
				zabbix_log(LOG_LEVEL_WARNING, "ER_LOCK_WAIT_TIMEOUT occurred and start retry case. query is %s", sql);
				result = (DB_RESULT)ZBX_DB_DOWN;
			}
		}
	}

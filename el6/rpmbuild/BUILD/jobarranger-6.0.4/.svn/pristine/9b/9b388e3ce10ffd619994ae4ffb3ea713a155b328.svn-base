...
{
    /// <summary>
    /// 条件分岐アイコン設定テーブルのDAOクラス
    /// </summary>
    public class RunIconIfDAO : BaseDAO
    {
        #region フィールド


        private string _tableName = "ja_run_icon_if_table";

        private string[] _primaryKey = { "inner_job_id"};

        private string _selectSql = "select * from ja_run_icon_if_table where 0!=0";

        private string _selectSqlByPk = "select * from ja_run_icon_if_table " +
                                        "where inner_job_id = ? ";

        private string _selectSqlByJobnet = "select * from ja_run_icon_if_table " +
        ...
        public RunIconIfDAO(DBConnect db)
        {
            _db = db;
        }
        #endregion

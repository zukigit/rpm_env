...
static int process_jarun()
{
    ...
    DB_RESULT result3;
    DB_RESULT result4;
    DB_RESULT session_result;
    DB_RESULT abort_flag_result;
    ...
    DB_ROW row4;
    DB_ROW session_row;
    DB_ROW abort_flag_row;
    double sec;
    zbx_uint64_t inner_job_id, inner_jobnet_id, inner_jobnet_main_id;
    int method_flag, job_type, test_flag;

    static pid_t pid_check = -1;
    JA_PID result_pid;
    static double next_execution_time_for_session = -1;
    int canSessionExecute = JA_FALSE;
    ...
    sec = zbx_time();
    result =
        DBselect("select inner_job_id, inner_jobnet_id, method_flag, job_type, test_flag, inner_jobnet_main_id from ja_run_job_table"
                 " where status = %d and method_flag <> %d ", JA_JOB_STATUS_READY, JA_JOB_METHOD_ABORT);
    ...
    while (NULL != (row = DBfetch(result)))
    {
        ...
        // zabbix_log(LOG_LEVEL_INFORMATION,"Debug : JobIcon:"ZBX_FS_UI64" inner_jobnet_main_id"ZBX_FS_UI64, inner_job_id, inner_jobnet_main_id);
        abort_flag_result = 
            DBselect("select jobnet_abort_flag from ja_run_jobnet_summary_table"
                " where inner_jobnet_id = "ZBX_FS_UI64, inner_jobnet_main_id);
                
        if(NULL != (abort_flag_row = DBfetch(abort_flag_result))){
            if(atoi(abort_flag_row[0])== JA_JOBENT_ABORT_FLAG_ON){
                // zabbix_log(LOG_LEVEL_INFORMATION,"Debug :JobIcon:"ZBX_FS_UI64" Jobnet:"ZBX_FS_UI64" JA_JOBENT_ABORT_FLAG is :%d ", 
                //                 inner_job_id, inner_jobnet_main_id,atoi(abort_flag_row[0]));
                DBfree_result(abort_flag_result);
                continue;
            }
        }
        DBfree_result(abort_flag_result);
    ...
    //check time for session check execution
    if(next_execution_time_for_session < zbx_time()) {
        next_execution_time_for_session = zbx_time() + 60;
        canSessionExecute = JA_TRUE;
    }
    ...
    if (pid_check == 0) {
        ...
        int size_of_session_processlist= 512;
        int* joblist = (int*)malloc(size_of_joblist * sizeof(int));
        pid_t* pid_list = (pid_t*)malloc(size_of_session_processlist * sizeof(int));
        ...
        if (next_execution_time_for_request < zbx_time()) {
            if(sql_flag == 0) {
                ...
                result3 = DBselect("select  f.before_value as host_name, e.inner_job_id, f.inner_jobnet_id, f.seq_no"
                                   " from"
        ...
        //check session's processes
        if(canSessionExecute) {
            session_result = DBselect(" select pid from ja_session_table jst, ja_run_job_table jrjt"
                                      " where jst.inner_job_id = jrjt.inner_job_id and jrjt.status = %d;", JA_JOB_STATUS_RUN);
            
            while(NULL != (session_row = DBfetch(session_result))) {
                if (count_pid > size_of_session_processlist) { 
                    size_of_session_processlist += 512;
                    pid_t* resized_pidlist = (pid_t*)realloc(pid_list, size_of_session_processlist * sizeof(int));
                    pid_list = resized_pidlist;
                }
                pid_list[count_pid] = atoi(session_row[0]);
                count_pid++;
            }
            jarun_session_check(pid_list, count_pid);
            DBfree_result(session_result);
        }
...
void main_jarun_loop()
{
    ...
    ja_alarm_watcher("ja_run");
    DBconnect(ZBX_DB_CONNECT_NORMAL);

    for (;;) {
        zbx_setproctitle("%s [processing data]",
                         ja_get_process_type_string(process_type));
        ja_alarm_timeout(CONFIG_RUN_TIMEOUT);
